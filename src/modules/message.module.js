...
export default class MessageModule extends Module {
  ...
  trigger() {
    ...
    const ofMessages = [
      ...
      "Определение программиста:живой организм, превращающий кофеин и пиццу в программное обеспечение.",
    ];
    ...
    const blockWithMessage = document.createElement("div");
    blockWithMessage.textContent = randomMessage;
    blockWithMessage.className = "block-with-message";
    document.body.append(blockWithMessage);

    // Удаление блока через некоторое время
    setTimeout(function () {
      if (blockWithMessage) {
        blockWithMessage.remove();
      }
    }, 5000);
  }
}
